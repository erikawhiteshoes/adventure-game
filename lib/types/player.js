var Player = function(options) {
  ...
  this.eat = function(item) {
    if(!item.flags.includes("edible")) {
      return "You can't eat it, Sucker.";
    }
    var self = this;
    item.effects.filter(function(effect) {
      return effect.action === "eat"; 
    }).forEach(function(effect) {
      Object.keys(effect.attributes).forEach(function(key) {
        if(typeof(effect.attributes[key]) !== "number") {
          return;
        }
        self.attributes[key] += effect.attributes[key];
      });
    });
    return ["You ate", item.name].join(' ');
  };
